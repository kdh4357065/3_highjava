...
//인터페이스이다. (내부 정렬 기준을 작성할 때 사용)(문자열 정렬에 쓰기 좋다)

//Comparator는 외부에 별도로 정렬 기준을 구현하고 싶을 때 구현하는 인터페이스이다.
//(외부 정렬 기준이라 한다.)(숫자 정렬에 쓰기 좋다)

//Comparable에서는 compareTo()메서드를 재정의 하고,
//Comparator에서는 compare()메셔드를 재정의 해야 한다.
...
public class ListSortTest {
	public static void main(String[] args) {
		...
		Collections.sort(list); // 오름차순으로 정렬 됨
...
class Desc implements Comparator<String> {
	...
	// 예) 오름차순일 경우 ==> 앞의 값이 크면 양수반환, 같으면 0반환, 앞의 값이 작으면 음수 반환하도록
	// 구현하면 된다.
