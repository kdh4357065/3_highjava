...
public class SetTest {
	public static void main(String[] args) {
		...
		//Set에 데이터를 추가할 ㄹ때도 add()메서드를 이용한다.
		hs1.add("DD");
		hs1.add("AA");
		hs1.add(2);
		hs1.add("CC");
		...
		//Set의 데이터 삭제하기 ==> remove(삭제할자료) ==> 반환값 : 삭제성공 true반환, 삭제실패 false반환
		//                 ==> clear() ==> 전체 데이터 삭제
...
//		System.out.println("Set 데이터 hs1 => " + hs1);
		...
		//그래서 데이터를 하나씩 차례로 업ㄷ기 위해서는 Iterator형 객체로 변환해야 한다.
		
		// - Set형의 데이터들은 Iterator형 객체로 변환하는 메서드 ==> iterator()
		...
		while(it.hasNext()) {
			//Iterator의 next()메서드
			//   ==> Iterator의 포이터를 다음번째 위치로 이동시킨 후 그 곳의 데이터를 꺼내서 반환한다.
