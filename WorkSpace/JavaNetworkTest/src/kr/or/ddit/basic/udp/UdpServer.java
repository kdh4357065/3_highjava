...
/*
 ...
 1. DatagramSocket : 데이터의 송수신과 관련된 작업을 수행한다.(우체분)
	...
	public static void main(String[] args) {
		try {
			...
			while(true){
				...
				//상대방잉 보낸 메시지 출력하기
				//- 상대방이 보낸 데이터는 수신용 패킷객체를 생성할 떄 지정한 byte형 배열
				//  수신용 패킷 객체의 getData()메서드를 통해서 구할 수 있다.
				//- 실제 읽어온 데이터의 길이는 수신용패킷 객체의 getLength()메서드를 통해서 구할 수 있다.
				
				//수신된 데이터는 byte형 배열로 오기 떄문에 이 데이터를 문자열로 변환해서 출력한다.
				...
				System.out.println("상대방이 보낼 메시지 : " + msg);
				System.out.println();
				//---------------------------------------------------------
				
				//상대방에게 메시지 보내기 (수신 받은 메시지를 그대로 송신한다.)
				
				//송힌할 데이터를 byte형 배열로 변환한다.
