...
public class JdbcTest01 {
	/*
	 ...
	 1. 드라이버 로딩 ==> 라이브러리를 사용할 수 있게 메모리로 읽어 들이는 작업
	    ==> JDBC API버전 4이상에서는 getConnection()메서드에서 자동을 로딩해 주기 때문에
	        생략할 수 있다.(그렇지만 우리는 생략하지 않고 사용할 예정임)
	    Class.forName("cracle.jdbc.driver.OracleDriver");
	...
	*/
	public static void main(String[] args) {
		...
		try {
			...
			//rs.next() ==> ResultSet객체의 데이터를 가리키는 포이터를 다음번째 레코드 위치로
